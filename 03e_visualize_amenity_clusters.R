################################################################################
# 03e - Visualize the Selected Amenity-Augmented Clusters
################################################################################
#
# Creates static and interactive maps for the silhouette-selected amenity
# sensitivity solution from 03d_amenity_cluster_sensitivity.R. Tentative names,
...
if (length(missing_files) > 0) {
  stop(
    ...
    ". Run 03d_amenity_cluster_sensitivity.R first.",
    call. = FALSE
  )
}
...
p_static <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste0(
      "Silhouette-selected k = ", selected_k,
