...
Williamson receives a second review because its appraisal system uses separate
accounts for some condominium, common-area, and reference properties. The
eligibility step joins WCAD legal descriptions, use codes, unit fields, and
property types to the candidate parcels. Exclusion, manual-review, and
quality-control tables are written before an excluded record can affect unit
calibration, corporate-ownership summaries, ACS allocation, or clustering.
