################################################################################
# 03d - Amenity Cluster Sensitivity Analysis
################################################################################
#
...
selected_k <- silhouette_recommendations %>%
...
cat("\nDiagnostics at the silhouette-recommended k:\n")
print(
  method_metrics %>%
    inner_join(
      silhouette_recommendations %>% select(specification, recommended_k),
      by = "specification"
    ) %>%
