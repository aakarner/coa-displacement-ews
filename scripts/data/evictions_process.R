################################################################################
# Process Eviction Filing Data to Hexagonal Grid
################################################################################
#
# Reads prepared all-JP eviction filing records and ArcGIS geocoding results from
# scripts/data/evictions_prepare.R, assigns geocoded defendant addresses to the
...
if (!file.exists(eviction_filings_file) || !file.exists(eviction_geocoded_file)) {
  stop(
    "Prepared eviction/geocode outputs not found. Run ",
    "scripts/data/evictions_prepare.R first.",
    call. = FALSE
  )
}
