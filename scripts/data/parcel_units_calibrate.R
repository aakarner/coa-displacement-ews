################################################################################
# Calibrate Residential Parcel Unit Counts
################################################################################
#
...
WCAD_PARCEL_MAP_FILE <- here::here(
  "data",
  "raw_parcels",
  "williamson",
  "wcad_parcels.rds"
...
RESIDUAL_PARCEL_REVIEW_FILE <- here::here(
  "config",
...
required_files <- c(
  ...
  WCAD_PROPERTY_FILE,
  WCAD_PARCEL_FILE,
  WCAD_PARCEL_MAP_FILE,
  HEX_GRID_FILE,
  RESIDUAL_PARCEL_REVIEW_FILE
)
...
wcad_supplement <- build_wcad_certified_residential_supplement(
  base_parcels = parcels_raw,
  property_file = WCAD_PROPERTY_FILE,
  parcel_map_file = WCAD_PARCEL_MAP_FILE,
  hex_grid_file = HEX_GRID_FILE,
  review_file = RESIDUAL_PARCEL_REVIEW_FILE
)
parcels_raw <- bind_rows(
  parcels_raw,
...
write_csv(
  ...
  file.path(
    OUTPUT_DIR,
    "williamson_certified_residential_supplement.csv"
  )
)
write_csv(
  wcad_supplement$audit,
  file.path(
    OUTPUT_DIR,
    "williamson_certified_residential_supplement_audit.csv"
  )
)
write_csv(
  wcad_supplement$summary,
  file.path(
    OUTPUT_DIR,
    "williamson_certified_residential_supplement_summary.csv"
  )
)
print_progress(
  paste0(
    "Added ",
    scales::comma(nrow(wcad_supplement$parcels)),
    " active certified WCAD residential parcels missing from the broad input."
  )
)
