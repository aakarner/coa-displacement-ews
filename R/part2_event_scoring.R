...
part2_event_components <- function(index_name) {
  specification <- list(
    sr_311_pressure_index = c("sr_311_smoke_signal_latest_12mo_per_100_units",
...
part2_event_component_inputs <- function(features, components, index_name) {
  if (!identical(components, part2_event_components(index_name))) {
...
part2_fit_event_scaling <- function(features, components, index_name, reference_date, preserved_scaling = NULL) {
  reference_date <- as.Date(reference_date)
  if (length(reference_date) != 1L || is.na(reference_date)) stop("Invalid event reference date.", call. = FALSE)
  inputs <- part2_event_component_inputs(features, components, index_name)
  ...
  signed <- if (identical(index_name, "sr_311_pressure_index")) "sr_311_smoke_signal_latest_12mo_rate_change_per_100_units" else character()
  unchanged <- setdiff(components, signed)
  if (!is.null(preserved_scaling)) {
    p <- preserved_scaling
    b <- p$bounds
    if (!is.list(p) || !p$schema_version %in% c("part2-event-scaling-v1", "part2-event-scaling-v2") ||
  ...
  list(schema_version = "part2-event-scaling-v2", index_name = index_name,
...
part2_apply_event_scaling <- function(features, scaling) {
  if (!is.list(scaling) || !identical(scaling$schema_version, "part2-event-scaling-v2") ||
  ...
  inputs <- part2_event_component_inputs(features, scaling$components, scaling$index_name)
  if ("analysis_as_of_date" %in% names(features) &&
  ...
  signed <- if (identical(scaling$index_name, "sr_311_pressure_index")) "sr_311_smoke_signal_latest_12mo_rate_change_per_100_units" else character()
